...
export async function generateBackupSnapshot(
  ...
  } else if (backupType === 'automated') {
    const dayName = now.toLocaleDateString('en-US', { weekday: 'short', timeZone: 'Asia/Karachi' });
    const dateName = now.toLocaleDateString('en-CA', { timeZone: 'Asia/Karachi' });
    const timeName = now.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Asia/Karachi' }).replace(':', '-');
    defaultName = `STH-Auto-Backup-${dateName}-${dayName}-${timeName}`;
  }

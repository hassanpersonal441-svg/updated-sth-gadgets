...
export function buildOnlinePaymentWhatsAppMessage(params: {
  customer_name: string;
  payment_reference: string;
  total_amount: number;
  payment_method: string;
}): string {
  const { customer_name, payment_reference, total_amount, payment_method } = params;

  return [
    `${EMOJI.WAVE} Hello *${customer_name}*!`,
    '',
    `${EMOJI.SPARKLES} I have made the online payment for my order.`,
    '',
    `${EMOJI.CUSTOMER} *Customer Name:* ${customer_name}`,
    `${EMOJI.MONEY_BAG} *Payment Amount:* PKR ${total_amount.toLocaleString('en-PK')}`,
    `${EMOJI.NUMBERS} *Payment Reference:* ${payment_reference}`,
    '',
    `${EMOJI.CARD} *Payment Method:* ${payment_method}`,
    '',
    `${EMOJI.SPARKLES} I am sending the payment screenshot for verification.`,
    '',
    `${EMOJI.PRAY} Thank you!`,
  ].join('\n');
}

export function buildPaymentConfirmedWhatsAppMessage(params: {
  customer_name: string;
  total_amount: number;
  payment_reference: string;
  order_number?: string;
}): string {
  const { customer_name, total_amount, payment_reference, order_number } = params;

  const orderInfo = order_number 
    ? `${EMOJI.NUMBERS} *Order:* #${order_number}\n` 
    : '';

  return [
    `${EMOJI.WAVE} Hello *${customer_name}*!`,
    '',
    `${EMOJI.CHECK} Your online payment of PKR ${total_amount.toLocaleString('en-PK')} has been successfully verified.`,
    '',
    `${EMOJI.NUMBERS} *Payment Reference:* ${payment_reference}`,
    '',
    orderInfo,
    `${EMOJI.SPARKLES} Your order has now been placed and is pending approval.`,
    '',
    `${EMOJI.PRAY} Thank you for shopping with STH Gadgets.`,
  ].join('\n');
}

export function buildPaymentRejectedWhatsAppMessage(params: {
  customer_name: string;
  payment_reference: string;
  total_amount: number;
  rejection_reason?: string;
}): string {
  const { customer_name, payment_reference, total_amount, rejection_reason } = params;

  const reasonText = rejection_reason 
    ? `${EMOJI.WAVE} *Reason:* ${rejection_reason}\n` 
    : '';

  return [
    `${EMOJI.WAVE} Hello *${customer_name}*!`,
    '',
    `${EMOJI.WAVE} We could not verify the online payment for your order.`,
    '',
    `${EMOJI.NUMBERS} *Payment Reference:* ${payment_reference}`,
    `${EMOJI.MONEY_BAG} *Amount:* PKR ${total_amount.toLocaleString('en-PK')}`,
    '',
    reasonText,
    `${EMOJI.SPARKLES} Please contact STH Gadgets if you need assistance.`,
    '',
    `${EMOJI.PRAY} Thank you.`,
  ].join('\n');
}

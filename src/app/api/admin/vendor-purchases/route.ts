...
export async function GET(req: NextRequest) {
  ...
  try {
    // Build both reads before awaiting so the profile and purchase list load together.
    ...
    const [{ data: profData }, { data: purchasesData, error: purchasesErr }] = await Promise.all([
      supabase.from('vendor_profiles').select('*').eq('name', 'Voltix Mobile').maybeSingle(),
      query,
    ]);

    const vendorProfile = profData ? { ...DEFAULT_VOLTIX_PROFILE, ...profData } : DEFAULT_VOLTIX_PROFILE;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const recordToInsert = {
      ...
      payment_status: ['unpaid', 'partial', 'paid'].includes(payment_status) ? payment_status : 'unpaid',
      payment_method: ['cash', 'bank_transfer', 'easypaisa', 'jazzcash', 'other'].includes(payment_method) ? payment_method : 'cash',

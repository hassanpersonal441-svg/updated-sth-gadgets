...
export async function POST(req: NextRequest) {
  try {
    ...
    const fullCatalogListStr = allProducts.map((p) =>
      `- ${p.name} | Category: ${p.category?.name || 'Accessories'} | Price: PKR ${p.price.toLocaleString()} ${p.old_price ? `(Was: PKR ${p.old_price.toLocaleString()})` : ''} | Stock: ${p.stock_status === 'in_stock' ? 'In Stock' : 'Limited'} | URL: /products/${p.slug}`
    ).join('\n');

    const matchingProductsStr = relevantProducts.map((p) => {
      const specs = (p.specifications || []).map((s) => `${s.label}: ${s.value}`).join(', ');
      return `- ${p.name} | Price: PKR ${p.price.toLocaleString()} ${p.old_price ? `(Was: PKR ${p.old_price.toLocaleString()})` : ''} | Stock: ${p.stock_status === 'in_stock' ? 'In Stock' : 'Limited'} | Specs: ${specs || 'N/A'} | URL: /products/${p.slug}`;
    }).join('\n');
...
5. OUT OF STOCK HANDLING:
   • P9 Headphones → "Currently out of stock. Our top alternative is Buds Pro 6 at PKR 1,699 with touch controls and excellent sound! 🎧"
   • 20,000mAh Power Banks → "Currently not in stock. We have 10,000mAh fast charging power banks (LMA LP-01 at PKR 2,650, Magnetic Wireless at PKR 3,500)! 🔋"
   • Smart Watches / Mobile Covers → "Coming soon! Meanwhile check our earbuds, power banks and speakers. ⚡"

━━━━━━━━━━ STORE POLICIES ━━━━━━━━━━
• Delivery Fee: PKR 200 flat nationwide.
• FREE Delivery: Orders PKR 5,000 or above.
...
`;
    ...
    if (intents.isTracking) {
      ...
      fallbackReply = u
        ? `20,000mAh power bank filhal stock mein nahi hai. Lekin 10,000mAh fast charging options available hain jese **LMA LP-01 (PKR 2,650)** aur **Magnetic Wireless (PKR 3,500)**! 🔋`
        : `20,000mAh is currently out of stock. We have great 10,000mAh fast charging power banks from **PKR 2,650**! 🔋`;
      relevantProducts = candidateProducts.filter((p) => p.name.toLowerCase().includes('power bank'));
    } else if (matchesAny(lower, ['p9'])) {
      fallbackReply = u
        ? `P9 Headphones filhal stock mein nahi hain. Hamari best-seller **Buds Pro 6 (PKR 1,699)** check karein — touch controls, amazing sound! 🎧`
        : `P9 headphones are currently out of stock. Check our popular **Buds Pro 6 at PKR 1,699** — touch controls and crystal audio! 🎧`;

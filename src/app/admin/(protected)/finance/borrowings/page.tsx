...
export default function BorrowedAmountsPage() {
  ...
  function handleSendWhatsApp(b: Borrowing) {
    ...
    const message = `STH Gadgets — Borrowing Record\n\nAssalam-o-Alaikum,\n\nThis is to confirm that an amount of Rs. ${b.borrowed_amount?.toLocaleString()} has been borrowed.\n\nLender: ${b.lender_name}\nAmount Borrowed: Rs. ${b.borrowed_amount?.toLocaleString()}\nDate: ${b.borrowing_date}${purposeInfo}${orderInfo}\n\nCurrent Remaining Balance: Rs. ${b.remaining_amount?.toLocaleString()}\n\nJazakAllah Khair.`;
                  ...
                  {borrowings.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="py-3.5 px-3 text-right font-bold text-white">
                        Rs. {Number(b.borrowed_amount).toLocaleString()}
                      </td>

                      <td className="py-3.5 px-3 text-right text-emerald-400 font-bold">
                        Rs. {Number(b.total_repaid).toLocaleString()}
                      </td>

...
async function getStats() {
  ...
  const [
    ...
    supabase.from('orders').select('subtotal, order_items(unit_price, purchase_price, quantity, line_total)').in('status', ['approved', 'pending_payment', 'processing', 'shipped', 'delivered']).limit(100),
    supabase.from('vendor_purchases').select('*').limit(50),
  ]);

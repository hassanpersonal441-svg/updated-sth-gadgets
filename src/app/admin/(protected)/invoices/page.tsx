                ...
                invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-[#0F1A2A] transition">
                    ...
                    <td className="py-3.5 px-4 text-right font-mono">
                      <div className="font-bold text-white">PKR {inv.grand_total.toLocaleString()}</div>
                      {inv.item_discount > 0 && (
                        <div className="text-[10px] text-amber-400 font-bold bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded mt-1 inline-block">
                          🏷️ Adjusted: -PKR {inv.item_discount.toLocaleString()}
                        </div>
                      )}
                    </td>
                    <td className="py-3.5 px-4 text-right font-mono text-[11px]">
                      <div className="text-emerald-400 font-bold">PKR {inv.amount_paid.toLocaleString()}</div>
                      <div className="text-rose-400">Bal: PKR {inv.remaining_amount.toLocaleString()}</div>
                    </td>

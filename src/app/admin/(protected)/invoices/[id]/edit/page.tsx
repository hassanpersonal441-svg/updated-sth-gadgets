...
export default function EditInvoicePage({
        ...
        if (res.ok) {
          ...
          if (inv.invoice_items && inv.invoice_items.length > 0) {
            const mappedItems: InvoiceFormItem[] = inv.invoice_items.map((it) => {
              const matchedProd = (prods as any[])?.find((p: any) => p.id === it.product_id);
              const catPrice = matchedProd ? Number(matchedProd.price) : Number(it.unit_price);
              return {
                product_id: it.product_id || null,
                product_name: it.product_name,
                product_image: it.product_image || null,
                quantity: it.quantity,
                catalog_price: catPrice,
  ...
  const handleAdjustSellingPrice = (index: number, newUnitPrice: number) => {
    setItems((prev) => {
      const copy = [...prev];
      const item = { ...copy[index] };
      const catPrice = item.catalog_price || item.unit_price;
      const qty = Math.max(1, parseInt(item.quantity as any) || 1);
      const enteredPrice = Math.max(0, newUnitPrice);

      if (catPrice > enteredPrice) {
        item.unit_price = catPrice;
        item.discount = (catPrice - enteredPrice) * qty;
        item.total = enteredPrice * qty;
      } else {
        item.unit_price = enteredPrice;
        item.catalog_price = enteredPrice;
        item.discount = 0;
        item.total = enteredPrice * qty;
      }
              ...
              <table className="w-full text-left text-xs">
                <thead className="border-b border-slate-800 bg-[#0C1420] font-bold text-silver-bright uppercase text-[11px]">
                  <tr>
                    ...
                    <th className="p-3 text-right w-36">Selling Price (PKR)</th>
                    <th className="p-3 text-right w-32">Discount (PKR)</th>
            ...
            <div className="rounded-2xl border border-slate-800/80 bg-[#0C1420] p-6 space-y-4 flex flex-col justify-between">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs mb-4">
                  ...
                  <div>
                    <label className="block font-bold text-silver-dim mb-1">Amount Paid (PKR)</label>
                    <input
                      type="number"
                      min={0}
                      value={amountPaid}
                ...
                <div className="rounded-xl border border-slate-800 bg-[#080D15] p-4 text-xs space-y-2 font-mono">
                  ...
                  <div className="flex justify-between pt-2 border-t border-slate-800 text-sm font-bold text-white">
                    ...
                    <span className="text-[#00C4CC]">PKR {grandTotal.toLocaleString()}</span>
                  </div>

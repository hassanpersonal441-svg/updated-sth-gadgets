  ...
  async function handleSystemReset(action: string) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        admin(data.message || 'Reset completed successfully!', 'Settings Reset');
      ...
      if (res.ok) {
        ...
        if (typeof errorData.error === 'string') {
          ...
          if (fieldErrors && Object.keys(fieldErrors).length > 0) {
            errorMsg = Object.entries(fieldErrors).map(([k, v]) => `${k}: ${(v as any[]).join(', ')}`).join('; ');
          } else {
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <button
                  ...
                  onClick={() => {
                    ...
                    admin('PWA App Install Banner triggered on screen!', 'PWA Banner');
                  }}
                ...
                <span className="text-[11px] text-silver-dim">
                  • Operating Currency: <strong className="text-amber-400">PKR</strong>
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-2 text-xs font-bold text-silver-dim">Rs.</span>
                <input
                  type="number"
                  min="0"
            ...
            <button
              ...
              onClick={() => {
                try {
                  ...
                  admin('Restarting Admin Panel Tour...', 'Admin Tour');
                } catch {
                  // ignore
                }
              }}
            ...
            <button
              ...
              onClick={() => {
                try {
                  ...
                  admin('Opening Storefront Customer Tour...', 'Customer Tour');
                } catch {
                  // ignore
                }
              }}
            ...
            <span
              className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold border ${
                settings.auto_rotate_products
                  ...
                  : 'bg-slate-800/60 border-slate-700 text-silver-dim'
              }`}
            >
              {settings.auto_rotate_products ? '● Active' : '○ Disabled'}
            </span>
          </div>

          <p className="text-xs text-silver-dim leading-relaxed">
            Jab enable ho, products automatically{' '}
            <span className="font-semibold text-[#00C4CC]">circular rotation</span>{' '}
            mein shift hote hain — #1 product aakhir mein chala jata hai, #2 upar aa jata hai, aur isi tarah sequence aage barhta hai.
            Aapka original sort_order sequence hamesha preserved rahta hai.
          </p>

          <label className="flex items-start gap-3 rounded-xl border border-slate-800 bg-[#080D15] p-3.5 cursor-pointer hover:border-slate-700 transition">
            <input
              type="checkbox"
              checked={Boolean(settings.auto_rotate_products)}
              onChange={(e) => update('auto_rotate_products', e.target.checked)}
              className="mt-0.5 h-4 w-4 rounded accent-[#00C4CC]"
            />
            <div>
              <span className="font-semibold text-xs sm:text-sm text-silver-bright block">
                Enable Product Sequence Rotation
              </span>
              <p className="text-[11px] text-silver-dim mt-0.5">
                Har interval ke baad products ka sequence ek step aage shift ho jata hai. Sabhi visitors ko ek hi waqt mein aik jaisa order dikhega.
              </p>
            </div>
          </label>

          {settings.auto_rotate_products && (
            <div className="space-y-3 rounded-xl border border-slate-800/80 bg-[#080D15] p-4">
              <label className="text-xs font-semibold text-silver-bright block">
                Rotation Interval (har kitni der baad shift ho)
              </label>
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                {[
                  ...
                  const isSelected = (settings.auto_rotate_interval_minutes ?? 60) === value;
                  return (
                    <button
                      key={value}
                      type="button"
                      onClick={() => update('auto_rotate_interval_minutes', value)}
                      className={`rounded-xl border py-2.5 px-2 text-xs font-bold transition cursor-pointer ${
                        isSelected
                          ? 'border-[#00C4CC] bg-[#00C4CC]/15 text-[#00C4CC]'
                          : 'border-slate-700 bg-[#080D15] text-silver-dim hover:border-slate-600 hover:text-silver-bright'
                      }`}
                    >
                      {label}
                    </button>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-800/60">
                <p className="text-[11px] text-silver-dim">
                  ...
                  <span className="text-[#00C4CC] font-semibold">
                    har {settings.auto_rotate_interval_minutes ?? 60} minutes baad
                  </span>{' '}
                  sequence shift hoga.
                </p>
                <Link
                  href="/"
                  target="_blank"
                  className="text-[11px] font-semibold text-[#00C4CC] hover:underline inline-flex items-center gap-1"
                >
                  Storefront par check karein ↗
                </Link>
              </div>
            </div>
          )}

...
import { getSettings, getActiveCategories, getAllActiveProducts, getAllActiveSeries } from '@/lib/data';
...
export default async function HomePage() {
  const [settings, categories, products, series] = await Promise.all([
    getSettings(),
    getActiveCategories(),
    getAllActiveProducts(48), // Limit initial load for performance
    getAllActiveSeries().catch(() => []), // Handle missing product_series table
  ]);
  ...
  return (
    <>
      ...
      {settings?.brand_voice_enabled !== false && (
        <BrandVoice enabled={true} />
      )}
              ...
              {products.slice(0, 20).map((p) => (
                <li key={p.id}>
                  <Link href={`/products/${p.slug}`}>
                    ...
                    <span> – Price: PKR {p.price.toLocaleString('en-PK')}</span>
                  </Link>
                </li>
              ))}
            </ul>

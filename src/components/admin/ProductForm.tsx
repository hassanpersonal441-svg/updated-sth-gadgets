...
import type { Category, Product, Specification, KeyFeature, BundleOffer, BundleOfferItem, ProductVariant } from '@/types/database';
import type { ProductSeries, ProductType } from '@/types/database';
...
export default function ProductForm({
  ...
  const [seriesOptions, setSeriesOptions] = useState<ProductSeries[]>(initialSeries);
  const [productType, setProductType] = useState<ProductType>(product?.product_type || (initialSeriesId ? 'series_model' : 'product'));
  const [seriesId, setSeriesId] = useState(product?.series_id || initialSeriesId || '');
  const [modelNumber, setModelNumber] = useState(product?.model_number || '');
  
  // Check if database has product_type column
  const [hasProductTypeColumn, setHasProductTypeColumn] = useState(true);

  useEffect(() => {
    if (!seriesOptions.length) fetch('/api/series').then((r) => r.json()).then((d) => setSeriesOptions(d.series || [])).catch(() => {});
  ...
  // Validate categoryId against available categoryList
  useEffect(() => {
    if (categoryList.length > 0) {
      const isValid = categoryList.some((c) => c.id === categoryId);
      if (!isValid) {
        const prodCatValid = product?.category_id && categoryList.some((c) => c.id === product.category_id);
        if (prodCatValid) {
          setCategoryId(product!.category_id!);
        } else if (!categoryId && categoryList[0]?.id) {
          setCategoryId(categoryList[0].id);
        } else if (categoryId && !isValid && categoryList[0]?.id) {
          setCategoryId(categoryList[0].id);
        }
      }
    }
  }, [categoryList, categoryId, product]);

  // Synchronize category with series if a series is selected
  useEffect(() => {
    if (productType === 'series_model' && seriesId && seriesOptions.length > 0) {
      const s = seriesOptions.find((opt) => opt.id === seriesId);
      if (s?.category_id && categoryList.some((c) => c.id === s.category_id)) {
        setCategoryId(s.category_id);
      }
    }
  }, [productType, seriesId, seriesOptions, categoryList]);
  ...
  const isDuplicateSlug = Boolean(
    slug.trim() &&
    storeProducts.some((p) => p.slug === slug.trim() && (!isEdit || p.id !== product?.id))
  );
  ...
  function handleNameChange(value: string) {
    ...
    if (!slugTouched) {
      ...
      if (!isEdit && storeProducts.length > 0 && baseSlug) {
        let unique = baseSlug;
        let c = 1;
        while (storeProducts.some((p) => p.slug === unique)) {
          c++;
          unique = `${baseSlug}-${c}`;
        }
        setSlug(unique);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const resolvedCategoryId = categoryList.some((c) => c.id === categoryId) ? categoryId : null;
    ...
    // Always include product_type to ensure proper filtering
    payload.product_type = productType;
    
    // Only include series-related fields if we're in series mode
    if (productType === 'series_model') {
      payload.series_id = seriesId || null;
      payload.model_number = modelNumber.trim() || null;
    }
    ...
    try {
      ...
      router.push(productType === 'series_model' && seriesId ? `/admin/series/${seriesId}` : '/admin/products');
      router.refresh();
    } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800 bg-[#0C1420] p-6 shadow-sm space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2 rounded-xl border border-slate-800 bg-[#080D15] p-4">
            <label className="mb-2 block text-xs font-semibold text-silver-dim">Product Type</label>
            <div className="flex flex-wrap gap-4 text-sm text-silver-bright">
              <label className="flex items-center gap-2"><input type="radio" checked={productType === 'product'} onChange={() => setProductType('product')} /> Standalone Product</label>
              <label className="flex items-center gap-2"><input type="radio" checked={productType === 'series_model'} onChange={() => setProductType('series_model')} /> Series Model</label>
            </div>
            {productType === 'series_model' && <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <select required value={seriesId} onChange={(e) => setSeriesId(e.target.value)} className="rounded-lg border border-slate-700 bg-[#0C1420] px-3 py-2 text-sm text-silver-bright"><option value="">Select series</option>{seriesOptions.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
              <input value={modelNumber} onChange={(e) => setModelNumber(e.target.value)} placeholder="Model number (optional)" className="rounded-lg border border-slate-700 bg-[#0C1420] px-3 py-2 text-sm text-silver-bright" />
            </div>}
          </div>
          <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="block text-xs font-semibold text-silver-dim">Product Slug (URL) *</label>
              {isDuplicateSlug && (
                <span className="text-[11px] font-medium text-amber-400">
                  ⚠️ Slug exists (auto-resolves on save)
                </span>
              )}
            </div>
            <input
              ...
              placeholder="e.g. 20000mah-fast-charging-power-bank"
              className={`w-full rounded-xl border px-4 py-2 text-sm text-silver-bright focus:outline-none ${
                isDuplicateSlug
            ...
            {isDuplicateSlug && (
              <p className="mt-1 text-[11px] text-amber-400/90">
                Another product is using this slug. We will automatically append a unique suffix if left as is.
              </p>
            )}
          </div>
        </div>
          ...
          <div>
            ...
            {categoryList.length === 0 && (
              <p className="mt-1 text-xs text-amber-400">
                No categories found. You can assign one later in Categories management.
              </p>
            )}
          </div>
          <div>
          ...
          {keyFeatures.map((kf, i) => (
            <div key={i} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 bg-[#080D15] p-3 rounded-xl border border-slate-800/80">
              ...
              <input
                ...
                className="w-full flex-1 rounded-lg border border-slate-700/80 bg-[#0C1420] px-3 py-2 text-xs sm:text-sm text-silver-bright focus:border-[#00C4CC] focus:outline-none"
              />
                              ...
                              onChange={(e) => {
                                ...
                                if (foundProd) {
                                  ...
                                  updateBundleItemField(
                                    ...
                                    iIndex,
                                    'detail',
                                    foundProd.short_description || (foundProd.price ? `PKR ${foundProd.price}` : '')
                                  );
                                }
                              }}
                            ...
                            >
                              ...
                              {storeProducts.map((p) => (
                                <option key={p.id} value={p.id}>
                                  {p.name} — PKR {p.price} {p.sku ? `(${p.sku})` : ''}
                                </option>
                              ))}
                            </select>
                          )}
      ...
      <div className="-mt-3 rounded-2xl border border-slate-800 bg-[#0C1420] p-6 shadow-sm space-y-4">
        ...
        <div className="pt-2 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 items-center">
          <label className="flex items-center gap-2.5 rounded-xl border border-slate-800 bg-[#080D15] px-4 py-2 text-xs font-semibold text-silver-bright cursor-pointer hover:border-slate-700">
            <input
              type="checkbox"

...
export default function RepaymentModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (amount > remaining + 0.01) {
      setErrorMsg(
        `Repayment amount (Rs. ${amount.toLocaleString()}) cannot exceed remaining balance (Rs. ${remaining.toLocaleString()})`
      );
      return;
    }
        ...
        <div className="my-4 rounded-xl border border-slate-800 bg-[#080D15] p-3.5 space-y-1.5 text-xs">
          <div className="flex justify-between text-slate-400">
            ...
            <span className="font-mono text-white font-semibold">
              Rs. {borrowing.borrowed_amount?.toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between text-slate-400">
            <span>Total Repaid:</span>
            <span className="font-mono text-emerald-400 font-semibold">
              Rs. {borrowing.total_repaid?.toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between pt-1 border-t border-slate-800 text-sm font-bold text-white">
            ...
            <span className="font-mono text-[#00C4CC]">
              Rs. {borrowing.remaining_amount?.toLocaleString()}
            </span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1">
              Repayment Amount (Rs.) <span className="text-rose-400">*</span>
            </label>
            <input
              type="number"
              required
